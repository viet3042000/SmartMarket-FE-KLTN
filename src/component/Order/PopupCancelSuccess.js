...
export default function PopUpCancelSuccess({ popupCancelSuccess = null, setPopupCancelSuccess = f => f }) {
  if (!popupCancelSuccess) {
    return (<></>);
  }
  if (popupCancelSuccess.goHome) {
    return (<Redirect to="/" />);
  }
  return (
    <>
      ...
      <div className="z-20 fixed top-20 left-0 right-0 w-full max-w-md m-auto" >
        <div className={styles.modalPopup}>
          <div className="relative w-full h-full flex flex-col items-center bg-white shadow rounded-sm">
            ...
            <div className="h-48 w-full flex flex-col justify-between items-center">
              <p className="text-4xl font-thin text-green-600">Hủy đơn hàng thành công</p>
              <div className="grid grid-rows-2 gap-0">
                ...
                <p className="border-2 border-black border-t p-2">{popupCancelSuccess.orderId}</p>
              </div>
            </div>
